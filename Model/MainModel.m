...
a_m   = AP(s,1);   %Transcription rate
a_p=AP(s,2);       %Translation rate
P_H0=AP(s,3);      %Hes5 repression threshold
TauH=AP(s,4);      %Time delay associated with Hes5 self repression loop
n_H=AP(s,5);       %Hill coefficient
u_m   = log(2)/30; %mRNA degredation rate min-1
u_p   = log(2)/90; %Protein degradation rate min-1

%Multicellular/intercellular parameters
P_ND0 = 21000; %Repression threshold associated with Notch-Delta signalling (coupling strength) 
n_ND  = 4;     %Hill coefficient
TauND = 140;   %Time delay associated with Notch-Delta signalling
%__________________________________________________________________________
